...
// Command IDs - must match resource.h
...
GameUI::GameUI() : m_gameLogic(GetDocument()->m_gameState)
{
...
void GameUI::DrawDominoPanel(CDC* pDC)
{
    ...
    CGameDoc* pDoc = GetDocument();
    for (int i = 0; i < pDoc->m_gameState.availableDominoes.size(); i++) {
        CRect dominoRect = GetDominoRect(i);
